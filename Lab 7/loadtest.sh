...
for ((i = 0; i < $numClients; i++)); do
  ./client "$ipaddress" "$port" "$codefile" "$loopNum" "$sleepTime" > "client_$i.txt" &
done
...
for ((i = 0; i < numClients; i++)); do
  responses=$(grep "Successful Responses:" "client_$i.txt" | cut -d ':' -f 2)
  echo "responses: $responses"
  throughput=$(grep "Throughput:" "client_$i.txt" | cut -d ':' -f 2)
  echo "throughput: $throughput"
  responseTime=$(grep "Average Response Time:" "client_$i.txt" | cut -d ':' -f 2)  # Corrected index here
  ...
  totalResponseTime=$(echo "$totalResponseTime + $responseTime" | bc)
...
# fi
if [ "$numClients" -gt 0 ]; then
  averageResponseTime=$(echo "scale=6; $totalResponseTime / $numClients" | bc)
  echo "$numClients $totalThroughput $averageResponseTime" >> "$data_file"
fi